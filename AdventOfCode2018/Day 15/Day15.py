...
def runSim(attack: int) -> int:
    ...
    def get_destination(unit_location: pos, battle_spaces: list[pos]):
        ...
        battle_spaces = [
            battle_space
            for battle_space in battle_spaces
            if battle_space in all_distances
        ]
    ...
    def takeTurn():
        ...
        for unit in all_units_at_start_of_turn:
            ...
            if unit.type == "E":
                ...
                raise (ValueError)
            targets = [t for t in targets if not t.died]
            if len(targets) == 0 and unit.type == "E":
                # print("Game Over")
                return "E"
            if len(targets) == 0 and unit.type == "G":
                return "G"
    ...
    def print_board(g: list[Unit], e: list[Unit], w: set[pos]):  # type: ignore
        for i in range(8):
            for j in range(7):
    ...
    for row, line in enumerate(contents):
        for col, c in enumerate(line):
            ...
            if c == "#":
                walls.add(location)
            if c == "E":
                elf = Unit("E", location, attack=attack)
                elves.append(elf)
    ...
    while True:
        ...
        if gameOver == "E":
            score = calculateScore(elves, goblins, round - 1)
            # print(score)
            return score
        if gameOver == "G":
            return 0
